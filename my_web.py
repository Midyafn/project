...
if option == 'Home' or option == '':
    ...
    #download template
    st.write("- prediksi yang dilakukan membutuhkan data sebanyak jumlah prediksi yang diinginkan ditambah data 1 bulan sebelumnya")
    st.write("- untuk memulai proses prediksi, silahkan download template terlebih dahulu")
    st.write("- contoh data pada template mohon dihapus dan diisi dengan data yang sesuai")
    st.write("")
    st.text("") 
    ...
    if uploaded_file is not None:
        ...
        st.write("Data telah berhasil dimuat, berikut adalah data yang digunakan untuk proses prediksi") 
        st.write(dataframe) 
        ...
        try: 
            ...
            df2['lag_2'] = df2['matahari'].shift(2)
            ...
            X = X.reshape((X.shape[0], n_seq, n_steps, n_features))
                    ...
                    #Loading Model
            model = keras.models.load_model('assets/new_6feature_cnnbilstm.h5')
            ...
            st.header('Prediksi rata-rata harian dengan CNN-Bidirectional LSTM')
            ...
            st.error('Data Tidak Sesuai', icon="🚨")
    ...
    if uploaded_file is not None:
        ...
        #dataframe['Tanggal'] = pd.to_datetime(dataframe['Tanggal'], format='%d-%m-%Y')
        try: 
            dataframe.columns = ['Tanggal','suhu_max', 'suhu_rata_rata', 'kelembaban(%)','matahari', 'angin_max', 'derajat_angin','angin']
            dataframe = dataframe.set_index('Tanggal')
            df2 = dataframe.dropna()
            #df_angin = df['ff_avg']
            ...
            #Loading Model
            model = keras.models.load_model('assets/new9010_8feature_gru_wind.h5')
        #Testing Model
            predictions = model.predict(X).flatten()
    ...
    b = st.number_input('Masukkan Suhu Rata-Rata', key = "b", min_value =25, max_value=40)
    c = st.number_input('Masukkan Derajat Arah Angin', key = "c", max_value=350)
    d = st.number_input('Masukkan Curah Hujan', key = "d", max_value=150)
    e = st.number_input('Masukkan Maksimum Kecepatan Angin', key = "e", max_value=50)
    f = st.number_input('Masukkan Penyinaran Matahari', key = "f", max_value=12)
    
    import pickle
    regression = pickle.load(open("assets/estimasi_angin_fin.pickle", "rb"))

    estimation = regression.predict([[a, b, c, d, e, f]])
    estimation = estimation[0]
    result = np.round(estimation, 2)
    st.text("Hasil Estimasi Kecepatan Angin hari berikutnya (m/s) :")
    #result = np.rint(estimation)
    if a == 0 or b == 0 or c==0 or f==0:
        st.text("0")
    else:
        st.text(result)
    ...
    g = st.number_input('Masukkan Suhu Maksimum', key = "g", min_value =26, max_value=40)
    h = st.number_input('Masukkan Suhu Rata-Rata', key = "h", min_value =25, max_value=38)
    i = st.number_input('Masukkan Curah Hujan', key = "i")
    
    import pickle
    regression_sun = pickle.load(open("assets/estimasi_matahari.pickle", "rb"))

    estimation_sun = regression_sun.predict([[g,h,i]])
